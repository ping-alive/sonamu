...
import { DlogStatus, DlogSearchField, DlogOrderBy } from "./dlog.enums";

export const DlogBaseSchema = z.object({
  id: z.number().int().nonnegative(),
  tid: z.string().max(50),
  phone_number: z.string().max(50),
  bankcode: z.number().int().nonnegative(),
  account: z.string().max(20),
  price: z.number().int().nonnegative(),
  name: z.string().max(50),
  paydt: z.string().length(19),
  uid: z.string().max(20),
  speed: z.number().int().nonnegative(),
  order_id: z.number().int().nullable(),
  res_msg: z.string().max(256),
  created_at: z.string(),
  status: DlogStatus,
});
export type DlogBaseSchema = z.infer<typeof DlogBaseSchema>;
export const DlogBaseListParams = z
  .object({
    num: z.number().int().nonnegative(),
    page: z.number().int().min(1),
    search: DlogSearchField,
    keyword: z.string(),
    orderBy: DlogOrderBy,
  ...
  .partial();
export type DlogBaseListParams = z.infer<typeof DlogBaseListParams>;

export const DlogSubsetA = z.object({
  id: z.number().int().nonnegative(),
  tid: z.string().max(50),
  phone_number: z.string().max(50),
  bankcode: z.number().int().nonnegative(),
  ...
  created_at: z.string(),
  status: DlogStatus,
  order_id: z.number().int().nonnegative().nullable(),
});
export type DlogSubsetA = z.infer<typeof DlogSubsetA>;

export type DlogSubsetMapping = {
  A: DlogSubsetA;
};
export const DlogSubsetKey = z.enum(["A"]);
export type DlogSubsetKey = z.infer<typeof DlogSubsetKey>;

export const dlogSubsetQueries: { [key in DlogSubsetKey]: SubsetQuery } = {
  A: {
    select: [
      "dlogs.id",
      "dlogs.tid",
